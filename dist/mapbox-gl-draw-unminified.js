...
var DEFAULT_SOURCE = 'mapbox-gl-draw';
...
function render() {
  ...
  var store = this;
  var sourceName = store.ctx.options.sourceName;
  var mapExists = store.ctx.map && store.ctx.map.getSource(sourceName) !== undefined;
  ...
  if (changed) {
    store.ctx.map.getSource(sourceName).setData({
